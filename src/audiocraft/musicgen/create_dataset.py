...
def gen_genre_ds(music_ds, label, step, audio_for_train=False):
    ...
    if step == "train":
        clear_if_exists(
            INPUT_PATH("textual-inversion-v2", "data", step, label, "encoded")
        )
    if step == "valid" or audio_for_train:
        clear_if_exists(
            INPUT_PATH("textual-inversion-v2", "data", step, label, "audio")
        )
        clear_if_exists(
            INPUT_PATH("textual-inversion-v2", "data", step, label, "encoded")
        )
    for idx, row in tqdm.tqdm(enumerate(music_ds)):
        ...
        if step == "valid" or audio_for_train:
            audio_path = os.path.join(
                "data", step, label, "audio", os.path.basename(path)
            )
...
if __name__ == "__main__":
    ...
    def make_data(ds, label, filter_func):
        def ds_fetch(step):
            ...
            return filtered_ds.take(min(MAX_NUM, len(filtered_ds))).cast_column(
                "path", Audio(sampling_rate=32000)
            )
    ...
    def save_json(ds: list, name):
        pd.DataFrame(ds).to_json(
            INPUT_PATH("textual-inversion-v2", name),
            force_ascii=False,
            orient="records",
        )
