...
class ConceptDataset(torch.utils.data.Dataset):
    ...
    def _preload_encoded(self, ds, pad_value: int = 0):
        enc_cache = []
        max_len = -1
        for row in ds:
    ...
    def _get_concepts(self):
        ...
        def collect_unique(batch):
            unique_values.update(
                [x.replace("\\", "").split("/")[2] for x in batch["audio_path"]]
            )
...
class ConceptDataModule(L.LightningDataModule):
    def __init__(
        self, ds, tokens_provider, tokens_ids, music_len: int = 255, batch_size: int = 5
    ):
        super().__init__()
        self.tokens_provider = tokens_provider
        self.tokens_ids = tokens_ids
