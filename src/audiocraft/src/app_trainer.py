...
import os
import wandb
from data import TokensProvider, ConceptDataModule, get_ds, TokensProvider, EvalDataModule
import uuid
...
class SaveEmbeddingsCallback(L.Callback):
    def __init__(self, save_path, concepts, tokens_ids_by_concept, tokens_num, weights, fad, n_epochs=10):
        super().__init__()
    ...
    def calc_fad(self, pl_module, trainer):
        ...
        for concept in self.concepts:
            ...
            with contextlib.redirect_stdout(io.StringIO()):
                fd_score = self.fad.score(INPUT_PATH(DATASET, 'data', 'train', f'{concept}', 'fad'), OUTPUT_PATH(DATASET, concept, 'temp'))
...
def run_training(cfg, wandb_logger):
    ...
    dm = EvalDataModule(ds, tokens_provider, tokens_ids_by_concept, music_len=249, batch_size=cfg.batch_size, base_dir=DATASET)
    model = TransformerTextualInversion(text_model, 
                                        tokenizer, 
                                        music_model, 
                                        text_conditioner, 
                                        tokens_ids, 
    ...
    quick_save_cl = SaveEmbeddingsCallback(MODELS_PATH(DATASET), cfg.concepts, tokens_ids_by_concept,cfg.tokens_num, text_model.shared.weight, fad, n_epochs=30)
    ...
    trainer = L.Trainer(callbacks=[quick_save_cl], enable_checkpointing=False, logger=wandb_logger, log_every_n_steps=10, max_epochs=250)
